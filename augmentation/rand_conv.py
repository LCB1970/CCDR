...
class RandConvModule(nn.Module):
    def __init__(self, net=None, kernel_size=3, in_channels=3, out_channels=3,
                 rand_bias=False, layer_num=3,
                 mixing=False,
                 identity_prob=0.0, distribution='kaiming_normal',
                 data_mean=(0.5, 0.5, 0.5), data_std=(0.5, 0.5, 0.5), clamp_output=True, share_parameter=False
                 ):
        """
        :param net:
        :param kernel_size:
        ...
        if share_parameter:
            kernel_size = kernel_size[0]
            e = RandConv2d(in_channels=in_channels, out_channels=out_channels,
                           padding=kernel_size // 2, kernel_size=kernel_size,
                           rand_bias=rand_bias, distribution=distribution,
                           clamp_output=self.clamp_output,
                           range_up=self.range_up,
                           range_low=self.range_low)
            e.randomize()
            self.randconv = nn.Sequential()
            for _ in range(layer_num):
                self.randconv.append(e)
        else:
            self.randconv = MultiScaleRandConv2d(in_channels=in_channels, out_channels=out_channels,
...
class RandConv2d(Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, rand_bias=True,
        ...
        :param kwargs:
        """
        super(RandConv2d, self).__init__(in_channels=in_channels, out_channels=out_channels,
                                         kernel_size=kernel_size, bias=rand_bias, **kwargs)
...
class MultiScaleRandConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_sizes,
        ...
        self.multiscale_rand_convs = nn.ModuleDict(
            {str(kernel_size): RandConv2d(in_channels, out_channels, kernel_size,
...
class data_dewhiten_layer(nn.Module):
    def __init__(self, data_mean, data_std):
        import torch
        super(data_dewhiten_layer, self).__init__()
        self.register_buffer('data_mean',
                             None if data_mean is None else torch.tensor(data_mean).reshape(3, 1, 1))
        self.register_buffer('data_std',
                             None if data_std is None else torch.tensor(data_std).reshape(3, 1, 1))
...
jitter = transforms.ColorJitter(0.3, 0.3, 0.3, 0.3)
gray = transforms.RandomGrayscale()


def multi_augment(input, data_mean, data_std):
    device = input.device
    data_mean = torch.tensor(data_mean).reshape(3, 1, 1).to(device)
    data_std = torch.tensor(data_std).reshape(3, 1, 1).to(device)
    input = dewhiten(input, data_mean, data_std)
    input = jitter(input)
    input = gray(input)
    return whiten(input, data_mean, data_std).contiguous()


def whiten(input, data_mean, data_std):
    return (input - data_mean) / data_std


def dewhiten(input, data_mean, data_std):
    return input * data_std + data_mean
